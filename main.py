...
file_handler = logging.FileHandler("./downloads/flatfile_to_xml_zip.log")
...
INPUT_FLAT_FILE = f"./downloads/metadata-dump-prod.csv"
XML_DIRECTORY = "./downloads/xml/"
XML_ZIP_FILE = "./downloads/xml_compressed"
...
def export_individual_xml(input_individual_xml, export_individual_file) -> None:
    """Export the given XML string to a file."""
    with open(export_individual_file, "w", encoding="utf-8") as f:
        f.write(input_individual_xml)
...
def compress_directory(input_xml_directory, output_zip_file) -> None:
    ...
    shutil.make_archive(output_zip_file, "zip", input_xml_directory)
        ...
        for metadata in filtered_batch.iter_rows(named=True):
            ...
            try:
                ...
                export_individual_xml(
                    ...
                    export_individual_file=f"{XML_DIRECTORY}/{ecatid_uuid}.xml",
                )
                logger.info(f"Completed processing metadata_id: {metadata['id']}")
            except Exception as e:
                logger.exception(f"Error processing metadata_id {metadata['id']} and ")

        logger.info(f"Starting compression of XML files...")
        compress_directory(
